...
def test_watcher_classifies_won_lost_pending():
    watcher = ResolutionWatcher(_StubPolyClient(0.99), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == LOST  # hedge side won -> leg lost

    watcher = ResolutionWatcher(_StubPolyClient(0.01), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == WON  # hedge side lost -> leg won

    watcher = ResolutionWatcher(_StubPolyClient(0.50), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == PENDING

    watcher = ResolutionWatcher(_StubPolyClient(None), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == PENDING
    assert watcher.resolve(None).status == PENDING


def test_watcher_asymmetric_threshold_edges():
    watcher = ResolutionWatcher(_StubPolyClient(0.97), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == LOST
    watcher = ResolutionWatcher(_StubPolyClient(0.98), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == LOST

    watcher = ResolutionWatcher(_StubPolyClient(0.011), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == PENDING
    watcher = ResolutionWatcher(_StubPolyClient(0.01), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == WON
    watcher = ResolutionWatcher(_StubPolyClient(0.005), won_threshold=0.99, lost_threshold=0.97)
    assert watcher.resolve("t1").status == WON
